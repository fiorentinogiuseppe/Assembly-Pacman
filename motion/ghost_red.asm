...
.macro move_red_ghost (%ghost_position)
	# Push to stack
	addi $sp, $sp, -24
	sw $s0, 0($sp)
	sw $s1, 4($sp)
	...
	# move right
	bne $a0, 0x0, move_red_ghost_not_right 
	move_red_right ($s0)
	j move_red_ghost_was_moved
	move_red_ghost_not_right:
	
	# move left
	bne $a0, 0x1, move_red_ghost_not_left 
	move_red_left ($s0)
	j move_red_ghost_was_moved
	move_red_ghost_not_left:
	
	# move up
	bne $a0, 0x2, move_red_ghost_not_up 
	move_red_up ($s0)
	j move_red_ghost_was_moved
	move_red_ghost_not_up:
	
	# move down
	bne $a0, 0x3, move_red_ghost_not_down 
	move_red_down ($s0)
	j move_red_ghost_was_moved
	move_red_ghost_not_down:
	
	move_red_ghost_was_moved:

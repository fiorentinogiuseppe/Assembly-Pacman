...
.macro move_orange_left (%orange_ghost_position)
	...
	sw $s2, 8($sp)
	sw $s3, 12($sp)
	sw %orange_ghost_position, 16($sp)
	sw $s4, 20($sp)
	# Push to stack
	...
	sw $s1, 16($sp)					# Fazer com que o registrador responsavel por apontar o fantasma laranja aponte para o no da esquerda
	lw $s2, 20($s1)			#
	...
	lw $s2, 8($sp)
	lw $s3, 12($sp)
	lw %orange_ghost_position, 16($sp)
	lw $s4, 20($sp)
	addi $sp, $sp, 24
	# Pop from stack
.end_macro
...
.macro move_orange_right (%orange_ghost_position)
	...
	sw $s1, 16($sp)					# Fazer com que o registrador responsavel por apontar o fantasma laranja aponte para o no da direita
	lw $s2, 20($s1)			#
...
.macro move_orange_up (%orange_ghost_position)
	...
	sw $s1, 16($sp)					# Fazer com que o registrador responsavel por apontar o fantasma laranja aponte para o no de cima
	lw $s2, 20($s1)			#
...
.macro move_orange_down (%orange_ghost_position)
	...
	sw $s1, 16($sp)					# Fazer com que o registrador responsavel por apontar o fantasma laranja aponte para o no de baixo
	lw $s2, 20($s1)			#
